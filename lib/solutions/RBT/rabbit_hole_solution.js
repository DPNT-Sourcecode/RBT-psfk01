export default class RabbitHoleSolution {
  ...
  _getUnicodeConnector(cells, row, col, rows, columns) {
    ...
    // A connector at (row, col) is between 4 cells:
    // top-left: (row-1, col-1), top-right: (row-1, col)
    // bottom-left: (row, col-1), bottom-right: (row, col)

    let hasTop = false, hasBottom = false, hasLeft = false, hasRight = false;

    // Top vertical wall: between top-left and top-right cells (column boundary above this connector)
    // Exists if there's a vertical wall between columns col-1 and col at row-1
    if (row > 0 && col > 0 && col < columns) {
      hasTop = cells[row - 1][col - 1].right || cells[row - 1][col].left;
    }

    // Bottom vertical wall: between bottom-left and bottom-right cells (column boundary below this connector)
    // Exists if there's a vertical wall between columns col-1 and col at row
    if (row < rows && col > 0 && col < columns) {
      hasBottom = cells[row][col - 1].right || cells[row][col].left;
    }

    // Left horizontal wall: between top-left and bottom-left cells (row boundary left of this connector)
    // Exists if there's a horizontal wall between rows row-1 and row at column col-1
    if (col > 0 && row > 0 && row < rows) {
      hasLeft = cells[row - 1][col - 1].bottom || cells[row][col - 1].top;
    }

    // Right horizontal wall: between top-right and bottom-right cells (row boundary right of this connector)
    // Exists if there's a horizontal wall between rows row-1 and row at column col
    if (col < columns && row > 0 && row < rows) {
      hasRight = cells[row - 1][col].bottom || cells[row][col].top;
    }
