export default class RabbitHoleSolution {
  rabbit_hole(rows, columns, digging_moves, rendering_options) {
    ...
    // Build ASCII output with scaling
    let out = "";
    ...
    for (let row = 0; row < rows; row++) {
      ...
      for (let col = 0; col < columns; col++) {
        ...
        out += (pillarIsSpace ? spaceChar : "+") + (cells[row][col].top ? hChar.repeat(hScale) : spaceChar.repeat(hScale));
      }
      out += "+\n";
    ...
    for (let col = 0; col < columns; col++) {
      out += "+" + (cells[rows - 1][col].bottom ? hChar.repeat(hScale) : spaceChar.repeat(hScale));
    }
    out += "+\n";
