export default class RabbitHoleSolution {
  ...
  _getUnicodeConnector(cells, row, col, rows, columns) {
    ...
    // Top vertical wall: between top-left and top-right cells
    // Exists if: right wall of (row-1, col-1) OR left wall of (row-1, col)
    if (row > 0 && col > 0 && cells[row - 1][col - 1].right) hasTop = true;
    if (row > 0 && col < columns && cells[row - 1][col].left) hasTop = true;
    
    // Bottom vertical wall: between bottom-left and bottom-right cells
    // Exists if: right wall of (row, col-1) OR left wall of (row, col)
    if (row < rows && col > 0 && cells[row][col - 1].right) hasBottom = true;
    if (row < rows && col < columns && cells[row][col].left) hasBottom = true;
    
    // Left horizontal wall: between top-left and bottom-left cells
    // Exists if: bottom wall of (row-1, col-1) OR top wall of (row, col-1)
    if (row > 0 && col > 0 && cells[row - 1][col - 1].bottom) hasLeft = true;
    if (row < rows && col > 0 && cells[row][col - 1].top) hasLeft = true;
    
    // Right horizontal wall: between top-right and bottom-right cells
    // Exists if: bottom wall of (row-1, col) OR top wall of (row, col)
